...
def build_and_upload_package(project_root: str, staging_bucket: str, package_name: str, package_version: str) -> str:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Both configs are uploaded to GCS and passed as GCS URIs to the inference script
...
'''
        ...
        # Upload to GCS (use inference-job-dist path)
        print("Uploading package to GCS...")
        bucket_name = staging_bucket.replace("gs://", "").split("/")[0]
        blob_path = "/".join(staging_bucket.replace("gs://", "").split("/")[1:]) + f"/inference-job-dist/{tarball}"
...
def run(config_file, inference_config, checkpoint, **kwargs):
    """
    Submit inference job to Vertex AI.
    
    Args:
        config_file: Path to vertex.yaml config file
        inference_config: Path to inference.yaml config file
        checkpoint: Path to model checkpoint (required, can be local or GCS)
        **kwargs: Additional overrides (inference_data_path, batch_size, etc.)
    """
    ...
    inference_config_path = str(pathlib.Path(__file__).parent.parent / inference_config)
    ...
    module_path = vertex_config.get("module_path", "inference")  # Default to inference module
    
    # Upload inference config with auto-versioning
    inference_config_uri = upload_config_with_auto_version(
        local_config_path=inference_config_path,
        package_name=package_name,
        package_version=package_version,
        base_gcs_path=f"{staging_bucket}/inference-job-config",
    )
    print(f"Uploaded inference config to {inference_config_uri}")
    
    # Upload vertex config to GCS (infrastructure settings)
    vertex_config_gcs = f"{staging_bucket}/inference-job-config/vertex.yaml"
    upload_file_to_gcs(config_file_path, vertex_config_gcs)
    print(f"Uploaded vertex config to {vertex_config_gcs}")
    
    # Handle checkpoint upload if local path
    checkpoint_uri = checkpoint
    if not checkpoint.startswith("gs://"):
        # Upload local checkpoint to GCS
        print(f"Uploading local checkpoint to GCS...")
        checkpoint_filename = os.path.basename(checkpoint)
        checkpoint_uri = f"{staging_bucket}/inference-job-checkpoints/{checkpoint_filename}"
        upload_file_to_gcs(checkpoint, checkpoint_uri)
        print(f"Uploaded checkpoint to {checkpoint_uri}")
    ...
    vertex_staging_bucket = f"{staging_bucket}/inference-job-output"
    print(f"Vertex Staging bucket for job outputs: {vertex_staging_bucket}")
    job_name = vertex_config.get("job_name", "set-pb-inference-job")
    
    aiplatform.init(project=project_id, location=location)
    job = aiplatform.CustomPythonPackageTrainingJob(
        display_name=job_name,
        python_package_gcs_uri=python_package_gcs_uri,
        python_module_name=f"{package_name}.{module_path}",  # e.g., src.inference
        container_uri=container_uri,
        staging_bucket=vertex_staging_bucket,
        labels={"package_version": package_version_label, "run_type": "inference"},
    )
    
    # Prepare inference script arguments
    # Both configs and checkpoint are uploaded to GCS and passed as GCS URIs
    inference_args = [
        "--config", inference_config_uri,
    ...
    for key, value in kwargs.items():
        if value is not None:
            inference_args.extend([f"--{key}", str(value)])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Repurchase Model Inference Runner on Vertex AI")
    parser.add_argument(
        "--config_file",
        type=str,
        default="pipeline/vertex_inference.yaml",
        help="Path to Vertex AI config (default: pipeline/vertex_inference.yaml)"
    )
    parser.add_argument(
        "--inference_config",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to model checkpoint (local or gs:// URI)"
    )
    parser.add_argument(
        "--inference_data_path",
        type=str,
        help="Override inference data path (optional)"
    )
    parser.add_argument(
        "--batch_size",
        type=int,
    ...
    parser.add_argument(
        "--top_k",
        type=int,
        help="Number of top predictions to return per customer"
    )
    parser.add_argument(
        "--tag",
        type=str,
        help="Optional tag for inference output directory"
    )
